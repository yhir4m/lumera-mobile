...
interface AnimalEditFormComponentProps {
  animal: Core.Animal;
  onSuccess?: () => void;
  onCancel?: () => void;
}

export default function AnimalEditFormComponent({
  animal,
  onSuccess,
  onCancel,
}: AnimalEditFormComponentProps) {
  const { selectedOrgId, selectedProductionUnitId, setAnimals } = useRancho();

  // Form states
  const [identifier, setIdentifier] = useState<string>(animal.identifier || '');
  const [displayName, setDisplayName] = useState<string>(animal.display_name || '');
  const [name, setName] = useState<string>(animal.name || '');
  const [tagNumber, setTagNumber] = useState<string>(animal.tag_number || '');
  const [siniigaTag, setSiniigaTag] = useState<string>(animal.siniiga_tag || '');
  const [birthDate, setBirthDate] = useState<string>(animal.birth_date || '');
  const [sex, setSex] = useState<'male' | 'female' | 'unknown'>(animal.sex || 'unknown');
  const [origin, setOrigin] = useState<'born' | 'purchased' | 'transferred' | 'imported' | 'unknown'>(animal.origin || 'unknown');
  const [ownershipType, setOwnershipType] = useState<string>(animal.ownership_type || '');
  const [purpose, setPurpose] = useState<string>(animal.purpose || '');
  const [purity, setPurity] = useState<string>(animal.purity || '');
  const [sireId, setSireId] = useState<string>(animal.sire_id || '');
  const [damId, setDamId] = useState<string>(animal.dam_id || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await coreService.updateAnimal(
        selectedOrgId,
      ...
      // Updating the real animal in current context list
      setAnimals((prev) => prev.map((a) => (a.id === animal.id ? updatedAnimal : a)));

      Alert.alert('Éxito', 'El animal ha sido actualizado correctamente.');
      if (onSuccess) {
        onSuccess();
      }
    } catch (err: any) {
      const apiMessage = err.response?.data?.error?.message;
      Alert.alert('Error', apiMessage || err.message || 'Ocurrió un error al actualizar el animal.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.header}>
        <Text style={styles.title}>Editar Registro de Animal</Text>
        <Text style={styles.subtitle}>Actualice los datos del animal en la unidad productiva.</Text>
      </View>
